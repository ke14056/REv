...
class USBDeviceManager {
    ...
    initPlannedRequestUI() {
        ...
        const readSettingsFromUI = () => {
            ...
            const next = {
                ...cur,
                maxGeneratorKW: maxGenInput ? Number(maxGenInput.value || cur.maxGeneratorKW) : cur.maxGeneratorKW,
                rampStepKW: rampStepInput ? Number(rampStepInput.value || cur.rampStepKW) : cur.rampStepKW,
                rampIntervalMs: rampIntervalInput ? Number(rampIntervalInput.value || cur.rampIntervalMs) : cur.rampIntervalMs,
        ...
        const doPlan = () => {
            ...
            if (!Number.isFinite(w) || w <= 0) {
                this.showPlannedRequestStatus('Enter a valid target watts value.', 'error');
                return null;
            }
            const targetKW = w / 1000;
            const planned = this.planPowerRequest(targetKW, s);
            this._plannedLastPlan = planned;
            const report = this.formatPlannedSimulationReport(planned, s);
            const t = planned.status === 'ok' ? 'success' : (planned.status === 'wait' ? 'warning' : 'error');
            this.showPlannedRequestStatus(report || planned.message, t);
